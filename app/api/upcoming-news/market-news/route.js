...
import MarketNewsModel from '@/lib/models/MarketNewsModel';
...
export async function POST(req) { // Use the POST function for fetching data
    try {
        ...
        const insertPromises = newsItems.map(async (item) => {
            ...
            const existingItem = await MarketNewsModel.findOne({ link: newsData.link });

            if (!existingItem) {
                // Only insert if it doesn't exist
                await MarketNewsModel.create(newsData);
            }
        });
...
export async function GET(req, res) {
    try {
        ...
        const news = await MarketNewsModel.find({}).sort({ pubDate: -1 }); // -1 for descending order
