...
export default function Page() {
    const [totalInvestment, setCurrentExpenses] = useState(10000); // Current cost of Car
    ...
    const calculateCarPlan = () => {
        ...
        // Calculate the future cost of the Car, adjusted for inflation
        const futureCarCost = totalInvestment * Math.pow(1 + inflationRateDecimal, years);

        // Calculate the lumpsum amount needed today to reach the future cost
        const lumpsumInvestment = futureCarCost / Math.pow(1 + annualReturnRate, years);

        // Calculate the monthly SIP amount needed to accumulate the future Car cost
        const monthlyReturnRate = annualReturnRate / 12; // Convert to monthly rate
        const months = years * 12; // Total number of months
        const sipInvestment = futureCarCost / ((Math.pow(1 + monthlyReturnRate, months) - 1) / monthlyReturnRate);
        ...
        for (let year = 1; year <= years; year++) {
            // Calculate the total future Car cost up to this year, adjusting for inflation
        ...
        setResult({
            totalInvestment,
            futureValue: Math.round(futureCarCost),
